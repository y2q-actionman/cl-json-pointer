...
(defun get-by-json-pointer (obj pointer &key (type t))
  (let ((parsed-ptr
	 (parse-json-pointer pointer)))
    (traverse-by-json-pointer obj type parsed-ptr nil)))

(defun exists-p-by-json-pointer (obj pointer &key (type t))
...
(defun set-by-json-pointer (obj pointer value &key (type t))
  (funcall (make-setter-by-json-pointer obj type pointer :update) value))

(defun add-by-json-pointer (obj pointer value &key (type t))
  (funcall (make-setter-by-json-pointer obj type pointer :add) value))

(defun delete-by-json-pointer (obj pointer &key (type t))
  (funcall (make-setter-by-json-pointer obj type pointer :delete)))

(defun remove-by-json-pointer (obj pointer &key (type t))
  (funcall (make-setter-by-json-pointer obj type pointer :remove)))


(define-setf-expander get-by-json-pointer (obj pointer &key (type t) &environment env)
